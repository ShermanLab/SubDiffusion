...
%   3. Bio-Formats plugin: http://imagej.net/Bio-Formats (You will need the
%      package and matlab toolbox as well).
...
%
% Written by Yonatan Golan 2014-2017 - golanyoni@gmail.com

%% Initialize variables and check that all needed files are available
    % Generate macropath which will be used for sending commands to imagej.
    % Do this in matlab's temporary directory
    macro_path = [tempdir,'ThunderStorm_From_Matlab.ijm'];
    
    % Find ImageJ.exe path
    % First try in the program files directory and then all over. This is
    % done to save time.
    [~,found_in_programfiles] = ...
        system('if exist "c:\program files\imagej\imagej.exe" (echo 1) else (echo 0)');
    found_in_programfiles = str2double(found_in_programfiles);
    if found_in_programfiles
        ImageJ_Path = 'c:\progra~1\imagej\imagej.exe';
    else
        disp('Searching for ImageJ.exe. This may take a while...')
        [Result, ImageJ_Path] = system('WHERE /R "c:\\" ImageJ.exe');
        if Result ~= 0
            disp('ImageJ.exe not found. Aborting...')
            return
        else
            % Take only the first location it was found in
            ImageJ_Path = ImageJ_Path(1:strfind(ImageJ_Path,char(10)));
            disp(['ImageJ.exe was found in: ',ImageJ_Path])
        end            
    end      
    
    % Check if bfGetReader is available
    if ~exist('bfGetReader.m')
        disp(['Error - bfGetReader.m is not available. You should '...
            'download it from the Bio-Formats project website']);
        return
    end
