function [Combined_mat_file] = Tracks_To_Cells()
...
%% Choose desired files and initialize variables
    [matfiles, path_name] = uigetfile('*.mat','multiselect','on','Choose track result files to convert to cell arrays');
...
%% Run over all files
    for matfiles_ind = 1:numel(matfiles)
        ...
        save([path_name,newfilename],'X','Y','dt','Sigma','Intensity','Offset','Uncertainty','Frames','Cell_Center')
    end
    dt = mean(All_dt);
    % Save the large list
    Combined_mat_file = [path_name,path_name(LastSlash+1:end-1),' All XYt'];
    save(Combined_mat_file,'All_X','All_Y','All_dt','All_Cells','All_Sigma','All_Intensity','All_Offset','All_Uncertainty','All_Frames','Cell_Centers','dt')
